...
function clipboard.clear()e={}end","lib.clipboard",nil,_ENV)if not __f then error(__err,0)end __f()
...
\", got \"..class.type(n),3)end end end end","lib.parameters",nil,_ENV)if not __f then error(__err,0)end __f()
...
o[s]=t[n+1][r]i[s]=h+r s=s+1 end end a:mapPixels(i,o)end","graphics.image",nil,_ENV)if not __f then error(__err,0)end __f()
...
return error(\"unexpected mode: \"..tostring(l))end end","graphics.Canvas",nil,_ENV)if not __f then error(__err,0)end __f()
...
for m=1,u<c and u or c do d[m]={i,n,a:sub(m+r,m+r)}l[m]=h+m end self:drawPixels(l,d)end","graphics.DrawingCanvas",nil,_ENV)if not __f then error(__err,0)end __f()
...
self:Canvas(a,o)end function ScreenCanvas:reset()local a={}\
for o=1,self.width*self.height do self.last[o]=a end end\
function ScreenCanvas:drawToTerminals(a,o,i)\
o=o or 0 i=i or 0 if type(a)~=\"table\"then return\
error(\"expected table terminals, got \"..class.type(a))end if\
type(o)~=\"number\"then return\
error(\"expected number x, got \"..class.type(o))end if type(i)~=\
\"number\"then return\
error(\"expected number y, got \"..class.type(i))end local n=1\
local s,h=self.pixels,self.last local r=self.width\
for d=1,self.height do local l=false for u=1,r do local c=s[n]local m=h[n]if\
c[1]~=m[1]or c[2]~=m[2]or c[3]~=m[3]then l=true h[n]=c end\
n=n+1 end\
if l then local u,c,m={},{},{}n=n-r for f=1,r do\
local w=s[n]u[f]=t[w[1]]or\"0\"c[f]=t[w[2]]or\"0\"\
m[f]=w[3]==\"\"and\" \"or w[3]n=n+1 end for n=1,#a do\
a[n].setCursorPos(o+1,i+d)\
...
function ScreenCanvas:drawToScreen(a,o)return self:drawToTerminal(term,a,o)end","graphics.ScreenCanvas",nil,_ENV)if not __f then error(__err,0)end __f()
...
nil end end","interfaces.IAnimation",nil,_ENV)if not __f then error(__err,0)end __f()
local __f,__err=load("\
...
Exception.throw(IncorrectParameterException,\"invalid side '\"..a..\"'\",2)end end\
...
Exception.throw(IncorrectParameterException,\"invalid side '\"..a..\"'\",2)end end","interfaces.IAttributeAnimator",nil,_ENV)if not __f then error(__err,0)end __f()
...
while\
t[a+1]and t[a+1].z<e.z do t[a+1],t[a]=e,t[a+1]a=a+1 end self:setChanged()break end end end","interfaces.IChildContainer",nil,_ENV)if not __f then error(__err,0)end __f()
...
function ISize:setHeight(e)parameters.check(1,\"height\",\"number\",e)\
if\
self.height~=e then self.height=e self.canvas:setHeight(e)\
self:setChanged()for t=1,#self.children do\
self.children[t]:onParentResized()end end return self end","interfaces.ISize",nil,_ENV)if not __f then error(__err,0)end __f()

local __f,__err=load("class\"Event\"{event=\"Event\"}function Event:is(e)\
return self.event==e end\
function Event:handle(e)self.handled=true self.handler=e end","events.Event",nil,_ENV)if not __f then error(__err,0)end __f()
...
return self.key==keys[e]or self.held[e]end","events.KeyboardEvent",nil,_ENV)if not __f then error(__err,0)end __f()
local __f,__err=load("class\"MiscEvent\"\
extends\"Event\"{event=\"MiscEvent\",parameters={}}\
function MiscEvent:MiscEvent(e,...)self.event=e self.parameters={...}end","events.MiscEvent",nil,_ENV)if not __f then error(__err,0)end __f()
...
function o.handle()o.handled=true self:handle()end return o end","events.MouseEvent",nil,_ENV)if not __f then error(__err,0)end __f()
local __f,__err=load("class\"TextEvent\"\
extends\"Event\"{event=\"TextEvent\",text=\"\"}function TextEvent:TextEvent(e,t)self.event=e self.text=t end","events.TextEvent",nil,_ENV)if not __f then error(__err,0)end __f()
...
function Animation:finished()return not self.frames[self.frame]end","core.Animation",nil,_ENV)if not __f then error(__err,0)end __f()
...
if i then\
local n=i.readAll()i.close()\
return self.resource_loaders[o](self,a,n,...)else\
Exception.throw(ResourceLoadException,\"Failed to open file '\"..a..\
\"': not found under '/'' or '\"..self.path..\"'\",2)end else\
...
i==\"timer\"and a.mouse and n[1]==a.mouse.timer then\
...
table.remove(a.threads,h)end end end end end","core.Application",nil,_ENV)if not __f then error(__err,0)end __f()
...
self.canvas=ScreenCanvas(t,a)self.width=t self.height=a end\
function Screen:getsTermEvents()for e=1,#self.terminals do\
if self.terminals[e]==term then return true end end return false end function Screen:setChanged(e)self.changed=e~=false\
...
#self.terminals+1]=e\
self.canvas:reset()return self:setChanged()end\
...
for a=1,#self.children do t[a]=self.children[a]end for a=1,#t do t[a]:update(e)end end","core.Screen",nil,_ENV)if not __f then error(__err,0)end __f()
...
self:IChildContainer()self.style=Style(self)self.canvas=DrawingCanvas(a,o)\
local i=DynamicValueVendor(self)i:addAttribute(\"x\",{},self.setX)\
i:addAttribute(\"y\",{},self.setY)i:addAttribute(\"z\",{},self.setZ)\
i:addAttribute(\"width\",{},self.setWidth)i:addAttribute(\"height\",{},self.setHeight)\
i:addAttribute(\"style\",{},self.setStyle)i:addAttribute(\"parent\",{},self.setParent)\
self.dynamic=i end\
...
if e and not class.typeOf(e,Sheet)and not\
class.typeOf(e,Screen)then\
Exception.throw(IncorrectParameterException(\
\"expected Sheet or Screen parent, got \"..class.type(e),2))end if e then e:addChild(self)else self:remove()end\
...
e:handle(self)end end end","core.Sheet",nil,_ENV)if not __f then error(__err,0)end __f()
local __f,__err=load("\
...
a[self.object.class][o]~=nil then return a[self.object.class][o]end return a[self.object.class][i]end","core.Style",nil,_ENV)if not __f then error(__err,0)end __f()
local __f,__err=load("\
...
local __f,__err=load("class\"DynamicValueException\"extends\"Exception\"\
function DynamicValueException:DynamicValueException(e,t)return\
self:Exception(\"DynamicValueException\",e,t)end","exceptions.DynamicValueException",nil,_ENV)if not __f then error(__err,0)end __f()
local __f,__err=load("\
class\"DynamicValueVendor\"{instance=nil,objects={},attributes={}}\
function DynamicValueVendor:DynamicValueVendor(e)\
if not class.isInstance(e)then\
Exception.throw(IncorrectConstructorException(\
self.class:type()..\" expects Instance instance when created, got \"..class.type(e),3))end self.instance=e self.objects={}self.attributes={}end\
function DynamicValueVendor:addAttribute(e,t,a,o)\
parameters.check(4,\"attribute\",\"string\",e,\"dependencies\",t and\"table\"or\"nil\",t,\"setter\",\
a and\"function\"or\"nil\",a,\"getter\",o and\"function\"or\"nil\",o)\
if self.attributes[e]then self.attributes[e].setter=a\
self.attributes[e].getter=o else\
self.attributes[e]={setter=a,getter=o,dependencies={},linked={}}end return self:setAttributeDependencies(e,t)end\
function DynamicValueVendor:setAttributeDependencies(e,t)\
parameters.check(2,\"attribute\",\"string\",e,\"dependencies\",t and\"table\"or\"nil\",t)local o=self.attributes local i=o[e]if not i then\
Exception.throw(DynamicValueException(\"no such attribute '\"..e..\"'\",2))end for a=1,#i.dependencies do o[i.dependencies[a]].linked[e]=\
nil end i.dependencies={}\
for a=1,#t do\
if not\
o[t[a]]then\
Exception.throw(DynamicValueException(\"no such dependency attribute '\"..e..\"'\",2))elseif i.linked[t[a]]then\
Exception.throw(DynamicValueException(\"cyclic reference '\"..\
t[a]..\"' <--> '\"..e..\"'\",2))elseif t[a]==e then\
Exception.throw(DynamicValueException(\"self reference of '\"..e..\"'\",2))end i.dependencies[a]=t[a]o[t[a]].linked[e]=true end self:updateAttribute(e)end\
function DynamicValueVendor:setAttributeSetter(e,t)\
parameters.check(2,\"attribute\",\"string\",e,\"setter\",t and\"function\"or\"nil\",t)if not self.attributes[e]then\
Exception.throw(DynamicValueException(\"no such attribute '\"..e..\"'\",2))end\
self.attributes[e].setter=t self:updateAttribute(e)end\
function DynamicValueVendor:setAttributeGetter(e,t)\
parameters.check(2,\"attribute\",\"string\",e,\"getter\",t and\"function\"or\"nil\",t)if not self.attributes[e]then\
Exception.throw(DynamicValueException(\"no such attribute '\"..e..\"'\",2))end\
self.attributes[e].getter=t self:updateAttribute(e)end\
function DynamicValueVendor:removeAttribute(e)\
parameters.check(1,\"attribute\",\"string\",e)if not self.attributes[e]then return end\
local t=self.attributes[e].dependencies local a=self.attributes[e].linked for n=1,#t do self.attributes[t[n]].linked[e]=\
nil end local o,i=next(a)while o do\
self:removeAttribute(o)o,i=next(a,o)end end function DynamicValueVendor:updateAttribute(e)\
parameters.check(1,\"attribute\",\"string\",e)\
return self:setAttribute(e,self:getAttribute(e))end\
function DynamicValueVendor:setAttribute(e,t)\
parameters.check(1,\"attribute\",\"string\",e)if not self.attributes[e]then\
Exception.throw(DynamicValueException(\"no such attribute '\"..e..\"'\",2))end\
local o=self.attributes[e]\
if o.setter then o.setter(self.instance,t)else self.instance[e]=t end local i,n=next(o.linked)while i do self:updateAttribute(i)\
i,n=next(o.linked,i)end end\
function DynamicValueVendor:getAttribute(e)\
parameters.check(1,\"attribute\",\"string\",e)if not self.attributes[e]then\
Exception.throw(DynamicValueException(\"no such attribute '\"..e..\"'\",2))end\
local t=self.attributes[e]\
if t.getter then return t.getter(self.instance)else return self.instance[e]end end function DynamicValueVendor:wrapSetter(e)\
parameters.check(1,\"attribute\",\"string\",e)\
return function(t,a)return self:setAttribute(e,a)end end\
DynamicValueVendor.set=DynamicValueVendor.setAttribute DynamicValueVendor.get=DynamicValueVendor.getAttribute","dynamic.DynamicValueVendor",nil,_ENV)if not __f then error(__err,0)end __f()
...
while a and(not i or#n<i)do s,a=e(a,o)n[#n+1]=s end return n end","interfaces.IHasText",nil,_ENV)if not __f then error(__err,0)end __f()
...
Style.addToTemplate(Button,{[\"colour\"]=512,[\"colour.pressed\"]=8,[\"textColour\"]=1,[\"textColour.pressed\"]=1,[\"horizontal-alignment\"]=1,[\"horizontal-alignment.pressed\"]=1,[\"vertical-alignment\"]=1,[\"vertical-alignment.pressed\"]=1})","elements.Button",nil,_ENV)if not __f then error(__err,0)end __f()
...
Style.addToTemplate(Checkbox,{[\"colour\"]=256,[\"colour.checked\"]=256,[\"colour.pressed\"]=128,[\"checkColour\"]=32768,[\"checkColour.pressed\"]=256})","elements.Checkbox",nil,_ENV)if not __f then error(__err,0)end __f()
...
self.canvas:clear(self.style:getField\"colour\")if self.onPreDraw then self:onPreDraw()end\
for n=1,#e do local i=e[n]if\
i:isVisible()then i:draw()i.canvas:drawTo(self.canvas,i.x,i.y)\
if\
i.cursor_active then t,a,o=i.x+i.cursor_x,i.y+i.cursor_y,i.cursor_colour end end end if t then self:setCursorBlink(t,a,o)end if self.onPostDraw then\
self:onPostDraw()end self.changed=false end end Style.addToTemplate(Container,{[\"colour\"]=1})","elements.Container",nil,_ENV)if not __f then error(__err,0)end __f()
...
self:onClick(e.button,e.x,e.y)end e:handle()elseif e:is(3)then if self.onHold then\
self:onHold(e.button,e.x,e.y)end e:handle()end end\
Style.addToTemplate(Draggable,{[\"colour\"]=512,[\"colour.pressed\"]=8,[\"textColour\"]=1,[\"textColour.pressed\"]=1,[\"horizontal-alignment\"]=1,[\"horizontal-alignment.pressed\"]=1,[\"vertical-alignment\"]=1,[\"vertical-alignment.pressed\"]=1})","elements.Draggable",nil,_ENV)if not __f then error(__err,0)end __f()
...
self:onClick(e.button,e.x,e.y)end e:handle()elseif e:is(3)then if self.onHold then\
self:onHold(e.button,e.x,e.y)end e:handle()end end\
Style.addToTemplate(Image,{[\"shader\"]=false,[\"shader.pressed\"]=false})","elements.Image",nil,_ENV)if not __f then error(__err,0)end __f()
...
a,o=next(t,a)end end end function KeyHandler:draw()end","elements.KeyHandler",nil,_ENV)if not __f then error(__err,0)end __f()


local __f,__err=load("class\"Panel\"extends\"Sheet\"{}function Panel:onPreDraw()\
self.canvas:clear(self.style:getField\"colour\")end\
Style.addToTemplate(Panel,{[\"colour\"]=256})","elements.Panel",nil,_ENV)if not __f then error(__err,0)end __f()
...
self:setScrollX(math.max(math.min(a+e.button,n-self:getDisplayWidth(r,d)),0))end elseif e:is(2)or e:is(3)then if e.x==self.width-1 and d or e.y==self.height-1 and r then e:handle()end end end\
if e:typeOf(MouseEvent)then\
...
Style.addToTemplate(ScrollContainer,{[\"colour\"]=1,[\"horizontal-bar\"]=128,[\"horizontal-bar.bar\"]=256,[\"horizontal-bar.active\"]=8,[\"vertical-bar\"]=128,[\"vertical-bar.bar\"]=256,[\"vertical-bar.active\"]=8})","elements.ScrollContainer",nil,_ENV)if not __f then error(__err,0)end __f()
...
Style.addToTemplate(Text,{[\"colour\"]=1,[\"textColour\"]=128,[\"horizontal-alignment\"]=0,[\"vertical-alignment\"]=3})","elements.Text",nil,_ENV)if not __f then error(__err,0)end __f()
...
Style.addToTemplate(TextInput,{[\"colour\"]=256,[\"colour.focussed\"]=256,[\"colour.highlighted\"]=2048,[\"textColour\"]=128,[\"textColour.focussed\"]=128,[\"textColour.highlighted\"]=1,[\"mask\"]=false,[\"mask.focussed\"]=false})","elements.TextInput",nil,_ENV)if not __f then error(__err,0)end __f()
